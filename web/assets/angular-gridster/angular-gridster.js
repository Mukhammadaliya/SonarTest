	...
	.directive('gridsterItem', ['$parse', 'GridsterDraggable', 'GridsterResizable', 'gridsterDebounce',
		function($parse, GridsterDraggable, GridsterResizable, gridsterDebounce) {
          ...
          var $getters = {};
					...
					if (optionsKey) {
            scope.$watchCollection(optionsKey, function(newVals, oldVals) {
              if(newVals) {
                for(var key in newVals) {
                  var newVal = newVals[key];
                  if(!$getters[key]) {
                    $getters[key] = $parse(optionsKey + '.' + key);
                  }
                  var val1 = $getters[key](scope);
                  if (typeof val1 === 'number' || (typeof val1 === 'boolean' && (key =='draggable' || key == 'resizable'))) {
                    item[key] = val1;
                  }
                }
              }
            ...
            var aspectFn = function(aspect) {
              ...
              if (typeof val === 'number' || (typeof val === 'boolean' && (aspect =='draggable' || aspect == 'resizable'))) {
                item[aspect] = val;
              }
            };
            ...
            if(item.resizable == false) resizable.toggle(false);
            else resizable.toggle(!gridster.isMobile && gridster.resizable && gridster.resizable.enabled);
					};
          scope.$watch(function() {
          ...
          //updateResizable();
					...
					var updateDraggable = function() {
						if(item.draggable == false) draggable.toggle(false);
            else draggable.toggle(!gridster.isMobile && gridster.draggable && gridster.draggable.enabled);
					};
					scope.$watch(function() {
