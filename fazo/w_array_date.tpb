create or replace type body Fazo_Schema.w_Array_Date is
  ...
  begin
    if Val.Count = 1 then
      return cast(Val(1) as timestamp);
    elsif Val.Count = 0 then
      raise No_Data_Found;
    else
      raise Too_Many_Rows;
  ...
  begin
    return Fazo_Schema.Fazo.To_Array_Timestamp(Val);
  end;
  ...
  begin
    ...
    if Val is not null then
      ...
      loop
        Out.Print('"' || Fazo_Schema.Fazo.Format_Date(Self.Val(i)) || '"');
        if i <> Val.Count then
          Out.Print(',');
        end if;
      end loop;
  ...
  begin
    ...
    if Val is not null then
      for i in 1 .. Val.Count
      loop
        result := result || '"' || Fazo_Schema.Fazo.Format_Date(Val(i)) || '"';
        if i <> Val.Count then
          result := result || ',';
        end if;
      end loop;
