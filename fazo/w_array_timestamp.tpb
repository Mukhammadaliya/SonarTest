create or replace type body Fazo_Schema.w_Array_Timestamp is
  ...
  begin
    return Fazo_Schema.Fazo.To_Array_Date(Val);
  end;
  ...
  begin
    ...
    if Val is not null then
      ...
      loop
        Out.Print('"' || Fazo_Schema.Fazo.Format_Timestamp(Self.Val(i)) || '"');
        if i <> Val.Count then
          Out.Print(',');
        end if;
      end loop;
  ...
  begin
    ...
    if Val is not null then
      for i in 1 .. Val.Count
      loop
        result := result || '"' || Fazo_Schema.Fazo.Format_Timestamp(Val(i)) || '"';
        if i <> Val.Count then
          result := result || ',';
        end if;
      end loop;
